...
Expected: The generator can render the requested service configuration; access still depends on actual routing, filtering and service state.
Verify: Downstream checks distinguish listening service, HTTP response and end-to-end network reachability.
Sources: ANSIBLE-PACKAGE, ANSIBLE-SERVICE, NGINX-HTTP, PROJECT
...
Verify: The separate generator checks certificate availability and the deployed endpoint.
Sources: PROJECT, ANSIBLE-SERVICE
