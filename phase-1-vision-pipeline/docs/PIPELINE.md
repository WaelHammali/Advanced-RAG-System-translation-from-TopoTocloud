...
texts (vertical offset for same-line texts) relative to 0.6 x the larger extent, reading order = name -> ip -> mask
top-to-bottom / left-to-right (else 0.6), compatibility name|ip 1.0, ip|mask 1.0, name|mask 0.6, others 0.
...
label) + 0.15 OCR conf. Two different labels claiming one link -> neither is applied. Unapplied labels go to `unresolved`.
...
* Suffix notation: only `.N` as the final octet with prefix /24../31. Network labels only as CIDR with zero host bits (`192.168.1.0/24`); a plain
...
* Multi-homed devices: scalar `network` block is null, use `addresses`. Direct (OCR) addresses are not tied to a link.
